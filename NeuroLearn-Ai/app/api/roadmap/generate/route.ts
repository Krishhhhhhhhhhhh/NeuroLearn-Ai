...
export async function POST(request: NextRequest) {
  try {
    ...
    const apiKeys = apiKey.split(',').map((key: string) => key.trim()).filter((key: string) => key.length > 0);
    ...
    if (!model) {
      ...
      return NextResponse.json(
        {
          error: `No available Gemini models could be initialized. Please ensure your API key has access to at least one model (gemini-2.5-flash, gemini-1.5-flash, gemini-1.5-pro, or gemini-pro).`,
          details: process.env.NODE_ENV === 'development' ? (lastError instanceof Error ? lastError.message : String(lastError)) : undefined,
        },
        { status: 503 }
      );
    }
...
Return ONLY the JSON object above. No other text.`;
    ...
    const generatedRoadmap: GeneratedRoadmap = {
      title: parsedResponse.title || slug.charAt(0).toUpperCase() + slug.slice(1).replace(/-/g, ' '),
      description:
      ...
      edges: edges.length > 0 ? edges : enrichedNodes.flatMap((node: any, index: number) => 
    ...
    if (errorMessage.includes('API_KEY_INVALID') || errorMessage.includes('API key not valid')) {
      return NextResponse.json(
        {
          error: 'Gemini API key is invalid or expired. Please check your API key configuration.',
    ...
    if (errorMessage.includes('not found') || errorMessage.includes('not supported')) {
      return NextResponse.json(
        {
          error: 'The requested Gemini model is not available. Your API key may not have access to this model. Try enabling gemini-pro or gemini-1.5-pro in Google AI Studio.',
          details: process.env.NODE_ENV === 'development' ? errorMessage : undefined,
        },
        { status: 503 }
      );
